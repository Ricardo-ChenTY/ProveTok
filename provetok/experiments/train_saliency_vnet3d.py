"""Train a VNet-like saliency model to predict union lesion masks."""
...
from ..models.saliency_vnet3d import SaliencyVNet3D, SaliencyVNet3DConfig, save_saliency_vnet3d
...
class TrainConfig:
    ...
    output_dir: str = "./outputs/train_saliency_vnet3d"
...
def _collect(cfg: TrainConfig, *, split: str, max_samples: int) -> Dict[str, Any]:
    ...
    for batch in dl:
        ...
        if stats["num_samples"] == 1 or (stats["num_samples"] % 25) == 0:
            print(f"[train_saliency_vnet3d] loaded {stats['num_samples']}/{max_samples} split={split}", flush=True)
...
@torch.no_grad()
def _eval(model: SaliencyVNet3D, dl: DataLoader, device: torch.device, *, topk_ratio: float) -> Dict[str, float]:
    model.eval()
    losses: List[float] = []
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train a VNet-like saliency model (union mask segmentation).")
    ...
    ap.add_argument("--output-dir", type=str, default="./outputs/train_saliency_vnet3d")
    args = ap.parse_args()
    ...
    model = SaliencyVNet3D(
        SaliencyVNet3DConfig(
            in_channels=1,
    ...
    best_val_score = -1e9
    best_path = Path(cfg.output_dir) / "saliency_vnet3d.pt"
    ...
    for epoch in range(int(cfg.epochs)):
        ...
        if score > best_val_score:
            ...
            save_saliency_vnet3d(str(best_path), model, extra={"meta": meta.to_dict(), "epoch": int(epoch)})

        print(
            f"[train_saliency_vnet3d] epoch={epoch} train_loss={history[-1]['train_loss']:.4f} "
    ...
    out_json = Path(cfg.output_dir) / "train_saliency_vnet3d.json"
