"""Train a 3D U-Net saliency model to predict union lesion masks.

This is a stronger alternative to `train_saliency_cnn3d.py` when Dice/IoU are too
close to zero on local gold masks. It is still lightweight enough to run on
resized 64^3 volumes.
"""
...
from ..models.saliency_unet3d import SaliencyUNet3D, SaliencyUNet3DConfig, save_saliency_unet3d
...
class TrainConfig:
    ...
    output_dir: str = "./outputs/train_saliency_unet3d"
...
def _collect(cfg: TrainConfig, *, split: str, max_samples: int) -> Dict[str, Any]:
    ...
    for batch in dl:
        ...
        if stats["num_samples"] == 1 or (stats["num_samples"] % 25) == 0:
            print(f"[train_saliency_unet3d] loaded {stats['num_samples']}/{max_samples} split={split}", flush=True)
...
def _augment_inplace(xb: torch.Tensor, yb: torch.Tensor, *, rng: np.random.RandomState, cfg: TrainConfig) -> None:
    ...
    if bool(cfg.augment_flips):
        # Independent random flips per-sample for diversity under tiny datasets.
...
@torch.no_grad()
def _eval(model: SaliencyUNet3D, dl: DataLoader, device: torch.device, *, topk_ratio: float) -> Dict[str, float]:
    model.eval()
    losses: List[float] = []
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train a 3D U-Net saliency model (union mask segmentation).")
    ...
    ap.add_argument("--output-dir", type=str, default="./outputs/train_saliency_unet3d")
    args = ap.parse_args()
    ...
    model = SaliencyUNet3D(
        SaliencyUNet3DConfig(
            in_channels=1,
    ...
    # Optional global pos_weight for BCE to handle severe class imbalance.
    pos_weight = None
    loss_fn_posw = None
    ...
    best_val_score = -1e9
    best_path = Path(cfg.output_dir) / "saliency_unet3d.pt"
    ...
    for epoch in range(int(cfg.epochs)):
        ...
        if score > best_val_score:
            ...
            save_saliency_unet3d(str(best_path), model, extra={"meta": meta.to_dict(), "epoch": int(epoch)})

        print(
            f"[train_saliency_unet3d] epoch={epoch} train_loss={history[-1]['train_loss']:.4f} "
    ...
    out_json = Path(cfg.output_dir) / "train_saliency_unet3d.json"
