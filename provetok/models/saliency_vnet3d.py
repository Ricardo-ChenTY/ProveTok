...
class _ConvGNAct(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, *, norm_groups: int, dropout: float, stride: int = 1):
        super().__init__()
        self.conv = nn.Conv3d(int(in_ch), int(out_ch), kernel_size=3, padding=1, stride=int(stride))
...
class SaliencyVNet3DConfig:
    """VNet-like residual encoder-decoder saliency model (learned downsampling)."""
...
class SaliencyVNet3D(nn.Module):
    def __init__(self, cfg: SaliencyVNet3DConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.downsample: nn.ModuleList = nn.ModuleList()
        ...
        for out_ch in chs:
            ...
            self.downsample.append(_ConvGNAct(int(out_ch), int(out_ch) * 2, norm_groups=int(cfg.norm_groups), dropout=float(cfg.dropout), stride=2))
            in_ch = int(out_ch) * 2

        bottleneck_ch = int(base * (2**int(depth)))
        self.bottleneck = _ResBlock(int(bottleneck_ch), int(bottleneck_ch), norm_groups=int(cfg.norm_groups), dropout=float(cfg.dropout))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        skips = []
        y = x
        for down, downsample in zip(self.down, self.downsample):
            y = down(y)
            skips.append(y)
            y = downsample(y)
    ...
    @classmethod
    def from_checkpoint(cls, ckpt: Dict[str, Any]) -> "SaliencyVNet3D":
        cfg_dict = ckpt.get("cfg") or {}
        cfg = SaliencyVNet3DConfig(**{k: cfg_dict[k] for k in cfg_dict.keys() if k in SaliencyVNet3DConfig.__annotations__})
        model = cls(cfg)
...
def save_saliency_vnet3d(path: str, model: SaliencyVNet3D, *, extra: Optional[Dict[str, Any]] = None) -> None:
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    payload: Dict[str, Any] = {"saliency_vnet3d": model.to_checkpoint()}
    if extra:
        payload["extra"] = dict(extra)
    torch.save(payload, str(p))


def load_saliency_vnet3d(path: str, *, map_location: str | torch.device = "cpu") -> SaliencyVNet3D:
    try:
    ...
    payload = ckpt.get("saliency_vnet3d") if isinstance(ckpt, dict) else None
    if not isinstance(payload, dict):
        raise ValueError(f"Invalid saliency checkpoint at {path!r}: missing 'saliency_vnet3d' dict")
