...
class _DoubleConv(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, *, norm_groups: int, dropout: float):
        ...
        self.net = nn.Sequential(
            _ConvGNAct(in_ch, out_ch, norm_groups=norm_groups, dropout=dropout),
            _ConvGNAct(out_ch, out_ch, norm_groups=norm_groups, dropout=dropout),
        )
...
class SaliencyUNet3DConfig:
    """3D U-Net saliency model producing a per-voxel lesion probability map.
...
class SaliencyUNet3D(nn.Module):
    def __init__(self, cfg: SaliencyUNet3DConfig):
        super().__init__()
        self.cfg = cfg
        ...
        # Define `depth` as the number of downsampling steps (MaxPool) before the bottleneck.
        # Example (depth=3): 64 -> 32 -> 16 -> 8 (bottleneck), then upsample back 8 -> 16 -> 32 -> 64.
        chs = [int(base * (2**i)) for i in range(int(depth))]
        for out_ch in chs:
            self.down.append(_DoubleConv(in_ch, int(out_ch), norm_groups=int(cfg.norm_groups), dropout=float(cfg.dropout)))
            self.pool.append(nn.MaxPool3d(kernel_size=2, stride=2))
            in_ch = int(out_ch)

        bottleneck_ch = int(base * (2**int(depth)))
        self.bottleneck = _DoubleConv(int(chs[-1]), int(bottleneck_ch), norm_groups=int(cfg.norm_groups), dropout=float(cfg.dropout))
        ...
        for skip_ch in reversed(chs):
            ...
            self.up.append(_DoubleConv(int(skip_ch) * 2, int(skip_ch), norm_groups=int(cfg.norm_groups), dropout=float(cfg.dropout)))
            cur = int(skip_ch)

        self.head = nn.Conv3d(int(cur), 1, kernel_size=1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Expect (B,1,D,H,W). For depth=d, input sizes should be divisible by 2^(d-1).
        if x.dim() != 5:
        ...
        for i, (upconv, up) in enumerate(zip(self.upconv, self.up)):
            ...
            if y.shape[2:] != skip.shape[2:]:
                # Defensive: allow odd shapes by center-cropping the larger tensor.
    ...
    @classmethod
    def from_checkpoint(cls, ckpt: Dict[str, Any]) -> "SaliencyUNet3D":
        cfg_dict = ckpt.get("cfg") or {}
        cfg = SaliencyUNet3DConfig(**{k: cfg_dict[k] for k in cfg_dict.keys() if k in SaliencyUNet3DConfig.__annotations__})
        model = cls(cfg)
...
def save_saliency_unet3d(path: str, model: SaliencyUNet3D, *, extra: Optional[Dict[str, Any]] = None) -> None:
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    payload: Dict[str, Any] = {"saliency_unet3d": model.to_checkpoint()}
    if extra:
        payload["extra"] = dict(extra)
    torch.save(payload, str(p))


def load_saliency_unet3d(path: str, *, map_location: str | torch.device = "cpu") -> SaliencyUNet3D:
    # See note in `saliency.py` for `weights_only` compatibility.
    try:
    ...
    payload = ckpt.get("saliency_unet3d") if isinstance(ckpt, dict) else None
    if not isinstance(payload, dict):
        raise ValueError(f"Invalid saliency checkpoint at {path!r}: missing 'saliency_unet3d' dict")
