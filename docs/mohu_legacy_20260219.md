...
- [x] M0123: pp.md §6.3 外部 baselines adapter 模板化（输出→pred_jsonl→可复现评测入口）
  - Ref: P0037
  - Context: 当前已有通用脚本（`to_pred_jsonl.py`/`normalize_pred_jsonl_ids.py`/`eval_external_predictions`），但缺少每个 baseline 的“最小可跑模板/样例”，导致评测入口难复现、容易卡在 id/字段对齐。
  - Acceptance:
    - `scripts/external/to_pred_jsonl.py` 的 `--format`/字段契约在文档中写死；
    - 为 CT2Rep/Dia-LLaMA/MvKeTR/3D-CT-GPT++/μ²Tokenizer 提供各自的 adapter 模板（可先做 text-only/推理输出转写，不要求集成训练代码）；
    - 提供 1-2 个最小样例文件，确保 `to_pred_jsonl -> normalize_pred_jsonl_ids -> eval_external_predictions` smoke 可一键跑通。
  - Verification: `python scripts/external/to_pred_jsonl.py --help && python scripts/external/normalize_pred_jsonl_ids.py --help && python -m provetok.experiments.eval_external_predictions --help`
  - Resolution: 新增 `docs/external_baselines_adapter.md`（baseline 模板+一键命令），新增 `scripts/external/make_dummy_pred_jsonl_from_manifest.py`（不依赖外部输出的 smoke），并补齐 `scripts/external/examples/`；在 `docs/experiment.md` 增加 E0222 并已通过 smoke。

- [x] M0124: pp.md §5.2 Split-DPO 闭环 gate 未通过：根因定位→修复→full 过闸
  - Ref: P0038, E0220, E0221
  - Context: 当前 `E0221` full 显示 DPO 在闭环指标（WeightedIssue）上出现变坏样本并拉高均值，与 pp.md “DPO dominates heuristic” 叙事不一致；需要把训练偏好构造/推理策略与 verifier 的硬规则（尤其 R1 no-citation）对齐。
  - Acceptance:
    - 增加可复现 debug 入口：对指定 `sample_id` 跑 heuristic vs DPO，输出每步 issues/blame/split 决策与 R1/R2/R3/R4 分解；
    - 修复后 `E0221 full` 过 gate：DPO `weighted_issue_mean <= heuristic_mean`，且 `r1_no_citation` 不上升（最好 worse=0）；
    - 产出 per-sample 差异摘要（用于 case study/审计）。
  - Verification: `python -m provetok.experiments.figX_split_dpo_closed_loop --help`
  - Resolution:
    - 根因：`run_provetok_agent` 在单个 finding 上可能连续执行 `split` 直到耗尽 `max_steps_per_finding`，但旧逻辑只在 `stopped_reason=="max_steps"` 时 fallback，导致该 finding 未写入 `accepted_frames/accepted_citations`。最终 `frames` 与 `citations` 的索引错位，产生伪 R1（no-citation）/伪变坏样本。
    - 修复：`provetok/agent/loop.py` 引入 `done_finding`，当步数耗尽但未 accept/despecify 时强制 fallback 到保守 de-specify，并保持每个 slot 都产出 1 帧，避免 frame<->citation 索引错位。
    - Debug 入口：`python -m provetok.experiments.figX_split_dpo_closed_loop --dataset-type manifest --manifest <manifest.jsonl> --split test --dpo-ckpt <split_policy_dpo.pt> --dpo-device cpu --dpo-require-blame --output-dir outputs/_debug_split_dpo --debug-sample-id <scan_hash>`，会写出 `debug_<scan_hash>.json`（含逐步 trace/issues）。
    - 验证：重跑 `E0221 full`（58 test），产物 `outputs/E0221-split_dpo_closed_loop_ct_rate_100g_full/figX_split_dpo_closed_loop_20260218_113501/figX_split_dpo_closed_loop.json`：worse=0，heuristic 与 DPO 的 `weighted_issue_mean` 均为 0。

- [x] M0125: pp.md §6.6 LoRA/PEFT 训练路径未落地：最小可复现 smoke
  - Ref: P0039
  - Context: `Llama2PCG` 已支持 `lora_adapter_path` 推理加载，但缺少可复现的 LoRA 训练脚本、依赖 pin 与 smoke 验证，导致 pp.md “7B+LoRA parity” 无法落地。
  - Acceptance:
    - 提供独立 Py3.11 环境的 LoRA smoke 训练脚本（只证明跑得通，不强行追求提升）；
    - 训练产物为可加载的 adapter 目录；
    - 提供一个端到端 smoke：加载 LoRA 后跑 1 个 agent 示例并确保 JSON 合同/解析不退化。
  - Verification: `python -c "from provetok.pcg.llama2_pcg import Llama2PCGConfig; print('cfg_ok')"`
  - Resolution:
    - 新增 Py3.11 LoRA 环境说明：`scripts/external/lora_env.md`（复用 `/data/conda_envs/radeval311`，安装 `peft` + `nibabel`）。
    - 新增 LoRA smoke 数据构建脚本：`scripts/external/build_lora_sft_jsonl_from_manifest.py`，产物示例 `outputs/E0223-lora_sft_smoke/sft.jsonl`。
    - 新增 LoRA SFT 训练脚本：`scripts/external/train_lora_pcg_sft.py`，产物示例 adapter 目录 `outputs/E0224-lora_sft_train_smoke/`（含 `train_summary.json`）。
    - 新增端到端验证：`scripts/external/smoke_lora_agent.py`，产物示例 `outputs/E0225-lora_agent_smoke/lora_agent_smoke.json`（可解析 JSON 合同并跑通 agent）。
...
- [x] M0122: pp.md §6.1 CT-RATE 官方 split 暂不实现（按当前 manifest split 先推进）
  - Ref: P0035
  - Context: pp.md 里建议复现 RRG-DPO 的 CT-RATE official split，但当前仓库与实验台账主要基于现有 `/data/provetok_datasets/ct_rate_100g/manifest.jsonl` 的 patient-level split；为避免数据协议大改导致实验不可比/返工，先按用户要求 defer。
  - Acceptance: 在 `docs/plan.md`/`docs/mohu.md` 明确标注“defer §6.1”，并保证新增实验默认使用现有 manifest 或 RRG-DPO 预处理 manifest（不引入新的 split 规则）。
  - Verification: `rg -n "P0035" docs/plan.md && rg -n "M0122" docs/mohu.md`
  - Resolution: 已按用户要求 defer（仅补齐 §6.2 可比预处理与 §6.4 论文级指标链路；官方 split 后续单开任务）。
