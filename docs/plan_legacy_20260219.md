...
- [x] P0035: pp.md §6.2 RRG-DPO 可比预处理链路落地（CT-RATE）
  - Linked claims: C0006
  - Definition of done: 提供 `scripts/data/preprocess_manifest_rrg_dpo.py`（0.75×0.75×1.5mm³ + 480×480×240 + dtype=float16）并产出标准 `manifest_rrg_dpo.jsonl`；`scripts/data/validate_manifest.py` 可校验 spacing/shape/dtype/ProtocolLock。
  - Verification: `python scripts/data/preprocess_manifest_rrg_dpo.py --help && python scripts/data/validate_manifest.py --help`
  - Touchpoints: `scripts/data/preprocess_manifest_rrg_dpo.py`, `scripts/data/validate_manifest.py`, `provetok/data/preprocess_rrg_dpo.py`, `docs/experiment.md`（E0213）

- [x] P0036: pp.md §6.4 论文级 Table1 指标+统计口径可复现（含 GREEN/RadCliQ 外部导出）
  - Linked claims: C0006
  - Definition of done: `scripts/paper/compute_paper_metrics.py` 产出可直接抄表的 JSON（bootstrap CI + paired Wilcoxon + Holm）；GREEN/RadCliQ 通过 Py3.11 RadEval 环境导出 `extra_metrics_jsonl` 再 merge。
  - Verification: `python scripts/paper/compute_paper_metrics.py --help && python scripts/external/compute_radeval_metrics_jsonl.py --help`
  - Touchpoints: `scripts/paper/compute_paper_metrics.py`, `scripts/external/compute_radeval_metrics_jsonl.py`, `scripts/external/radeval_env.md`, `docs/experiment.md`（E0215–E0218）

- [x] P0037: pp.md §6.3 外部 baselines 适配模板化（输出→pred_jsonl→可复现评测入口）
  - Linked claims: C0006
  - Definition of done: 为每个外部 baseline 提供“输出转 pred_jsonl”的模板与最小样例；`eval_external_predictions` 能对 pred_jsonl 做 post-hoc citations 并输出 proof 指标；id 不一致可用 `normalize_pred_jsonl_ids.py` 归一化。
  - Verification: `python scripts/external/to_pred_jsonl.py --help && python -m provetok.experiments.eval_external_predictions --help`
  - Touchpoints: `scripts/external/to_pred_jsonl.py`, `scripts/external/normalize_pred_jsonl_ids.py`, `provetok/experiments/eval_external_predictions.py`, `docs/experiment.md`

- [x] P0038: pp.md §5.2 Split-DPO 真实数据闭环：trace→pair→训练→推理→对比 gate
  - Linked claims: C0006
  - Definition of done: `train_split_policy_dpo` 在 manifest(train) 上产出 ckpt；`figX_split_dpo_closed_loop` 在 manifest(test) 上证明 DPO 相对 heuristic 的 `weighted_issue` 至少 non-worse（并输出 per-sample 变坏样本定位）。
  - Verification: `python -m provetok.experiments.train_split_policy_dpo --help && python -m provetok.experiments.figX_split_dpo_closed_loop --help`
  - Touchpoints: `provetok/experiments/train_split_policy_dpo.py`, `provetok/experiments/figX_split_dpo_closed_loop.py`, `provetok/bet/split_policy_dpo.py`, `docs/experiment.md`（E0220–E0221）

- [x] P0039: pp.md §6.6 LoRA/PEFT 最小可复现训练路径（先 smoke）
  - Linked claims: C0006
  - Definition of done: 提供 LoRA smoke 训练脚本（独立 Py3.11 环境）并产出 adapter；`Llama2PCGConfig(lora_adapter_path=...)` 可加载并跑通 1 个端到端 agent smoke。
  - Verification: `python -c "from provetok.pcg.llama2_pcg import Llama2PCGConfig; print('cfg_ok')"`
  - Touchpoints: `provetok/pcg/llama2_pcg.py`, `docs/experiment.md`, `scripts/external/*`
