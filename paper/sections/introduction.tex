...
Groundedness is not an after-the-fact visualization: every generated frame must ``carry its proof'' (a verifier-checkable evidence chain: citations + trace), otherwise it is rejected by a verifier that triggers refinement or calibrated refusal.
...
We do not propose a new imaging backbone; instead we contribute two reusable mechanisms and a scriptable audit suite.
Our contributions are:
\begin{itemize}
  \item \textbf{BET-Alg: budgeted evidence tokenization as an algorithm.} We formulate evidence tokenization as cost-aware set selection over explicit 3D support regions and instantiate an auditable greedy refinement algorithm (BET-Alg) that is empirically near-oracle under matched budgets (Table~\ref{tab:oral-minset}).
  \item \textbf{Proof-carrying generation as a contract.} We define a hard output contract (frames, citations, verifier traces, calibrated refusal) that couples generation with evidence binding under a unified budget $\Btotal=\Benc+\Bgen$ (Figure~\ref{fig:system}) and isolate its causal contribution via contract ablations (Table~\ref{tab:oral-minset}).
  \item \textbf{Claim-level proof rules and statistics.} We implement scriptable claim-level tests with pre-registered protocols (matched budgets, paired bootstrap, Holm families) so key claims are machine-auditable rather than qualitative.
\end{itemize}
